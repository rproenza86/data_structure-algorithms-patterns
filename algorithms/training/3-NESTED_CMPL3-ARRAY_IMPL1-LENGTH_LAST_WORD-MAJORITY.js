...
/*
Brute force sol:
0- create var to hold the letters count
1- loop on the string char by char starting in the fist position
2- assume first no empty char is a word start and increase counter
3- if a space is found reset counter to 0 and start step 2 when found a new letter

Improved solution:
0- create var to hold the letters count
1- loop on the string char by char starting in the end of the string
2- if first char is empty return 0 assume string is empty
3- increase counter til find and space, then stop loop and return counter
*/
 ...
 */
// not optimized O(n). Auxiliary Space : O(n).
module.exports = { 
	...
	majorityElement : function(A){
	   ...
	   var auxArray = [];
	   var biggerOccurrence = 0, majorityElement;
      ...
      auxArray.map(function(occurrence, index){
           if(auxArray[index] > biggerOccurrence){
               biggerOccurrence = auxArray[index];
               majorityElement = index;
           }
       });
       
       return Math.round(aLength/2);
	}
};

// Better sol: Time Complexity: O(n*n). Auxiliary Space : O(1).
	 function majorityElement(A){
	    if(A.length === 1) return A[0];
	    var biggerOccurrence = 1, majorityElement = A[0];
	    
	   for(var i = 0, aLength = A.length; i < aLength; i++){
	       var inbiggerOccurrence = 1;
	       ...
	       if(biggerOccurrence < inbiggerOccurrence) {
	           biggerOccurrence = inbiggerOccurrence;
	           majorityElement = A[i];
	       }
	   }
...
/*
METHOD 4 (Using Hashmap) :This method is somewhat similar to Moore voting algorithm in terms of time complexity, but in this case there is no need of 
second step of Moore voting algorithm.But as usual, here space complexity becomes O(n).
In Hashmap(key-value pair), at value,maintain a count for each element(key) and whenever count is greater than half of array length, we are just 
returning that key(majority element).Thanks Ashwani Tanwar,Karan Malhotra for suggesting this.
